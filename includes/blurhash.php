...
function fh_seo_attachment_set_bluehash( $attachment_id, $components_x = 4, $components_y = 4 ) {
	...
	$image = imagecreatefromstring( file_get_contents( $path ) ); // phpcs:ignore
	$width = imagesx( $image );
 ...
 * @return array|bool Returns the array of the bluhash image color pixels on success.
 */
function fh_seo_attachment_decode_bluehash( $attachment_id, $width = 300, $height = 300 ) {
	...
	for ( $y = 0; $y < $height; ++$y ) {
		for ( $x = 0; $x < $width; ++$x ) {
			[ $r, $g, $b ] = $pixels[ $y ][ $x ]; // phpcs:ignore Generic.Arrays.DisallowShortArraySyntax.Found
